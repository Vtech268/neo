   ...
   async: async (m, {
      ...
      try {
         ...
         if (m.quoted && /Siapakah aku?\?/i.test(m.quoted.text)) {
            if (!(id in client.whoami) && /Siapakah aku?\?/i.test(m.quoted.text)) return client.reply(m.chat, Utils.texted('bold', `🚩 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}whoami_ untuk mendapatkan soal baru.`), m)
            if (m.quoted.id == client.whoami[id][0].key.id) {
