   ...
   async: async (m, {
      ...
      try {
         ...
         if (m.quoted && /songclue/i.test(m.quoted.text)) {
            if (!(id in client.whatsong) && /songclue/i.test(m.quoted.text)) return client.reply(m.chat, Utils.texted('bold', `🚩 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}whatsong_ untuk mendapatkan soal baru.`), m)
            if (m.quoted.id == client.whatsong[id][0].key.id) {
               ...
               if (body.toLowerCase() == json.title.toLowerCase()) {
                  await client.sendSticker(m.chat, await Utils.fetchAsBuffer('./media/sticker/true.webp'), m, {
                     ...
                     users.point += reward
                     if (client.whatsong?.[id]?.[2]) clearTimeout(client.whatsong[id][2])
