   ...
   async: async (m, {
      ...
      try {
         ...
         if (m.quoted && /brainwhat/i.test(m.quoted.text)) {    
            if (!(id in client.brainout) && /brainwhat/i.test(m.quoted.text)) return client.reply(m.chat, Utils.texted('bold', `🚩 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}brainout_ untuk mendapatkan soal baru.`), m)
            if (m.quoted.id == client.brainout[id][0].key.id) {
               ...
               if (body.toLowerCase() == json.jawaban.toLowerCase()) {
                  await client.sendSticker(m.chat, await Utils.fetchAsBuffer('./media/sticker/true.webp'), m, {
                     ...
                     users.point += reward
                     if (client.brainout?.[id]?.[2]) clearTimeout(client.brainout[id][2])
